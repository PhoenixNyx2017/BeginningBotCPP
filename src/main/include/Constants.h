...
namespace GeneralConstants {
    ...
    const SDSModuleType mk3_standard = SDSModuleType(
            0.1016,
            (14.0 / 50.0) * (28.0 / 16.0) * (15.0 / 60.0),
            true,
            (15.0 / 32.0) * (10.0 / 60.0),
            true);
    const SDSModuleType mk3_fast = SDSModuleType(
            0.1016,
    ...
    const SDSModuleType mk4_l1 = SDSModuleType(
            0.10033,
            (14.0 / 50.0) * (25.0 / 19.0) * (15.0 / 45.0),
            true,
            (15.0 / 32.0) * (10.0 / 60.0),
            true);
    const SDSModuleType mk4_l2 = SDSModuleType(
            0.10033,
            (14.0 / 50.0) * (27.0 / 17.0) * (15.0 / 45.0),
            true,
            (15.0 / 32.0) * (10.0 / 60.0),
            true);
    const SDSModuleType mk4_l3 = SDSModuleType(
            0.10033,
            (14.0 / 50.0) * (28.0 / 16.0) * (15.0 / 45.0),
            true,
            (15.0 / 32.0) * (10.0 / 60.0),
            true);
    const SDSModuleType mk4_l4 = SDSModuleType(
            0.10033,
    ...
    const SDSModuleType mk4i_l1 = SDSModuleType(
            0.10033,
            (14.0 / 50.0) * (25.0 / 19.0) * (15.0 / 45.0),
            true,
            (14.0 / 50.0) * (10.0 / 60.0),
            false);
    const SDSModuleType mk4i_l2 = SDSModuleType(
            0.10033,
            (14.0 / 50.0) * (27.0 / 17.0) * (15.0 / 45.0),
            true,
            (14.0 / 50.0) * (10.0 / 60.0),
            false);
    const SDSModuleType mk4i_l3 = SDSModuleType(
            0.10033,
            (14.0 / 50.0) * (28.0 / 16.0) * (15.0 / 45.0),
            true,
            (14.0 / 50.0) * (10.0 / 60.0),
    ...
    const double kMaxTranslationalVelocity = ((6380.0) / 60) * kSDSModule.wheelDiameter * M_PI
            * kSDSModule.driveReduction;


    const double kMaxRotationalVelocity = (kMaxTranslationalVelocity)
    ...
    const frc::Rotation2d kFrontLeftOffset = frc::Rotation2d(units::degree_t {103.711}); // module 1
//     public static final Rotation2d kFrontLeftOffset = Rotation2d.fromDegrees(106.172); // 283.447.unaryMinus() // -76.201
    const frc::Translation2d kFrontLeftPosition = frc::Translation2d(units::meter_t {kTrackwidthMeters / 2.0},
    ...
    const frc::Rotation2d kFrontRightOffset = frc::Rotation2d(units::degree_t {318.427}); // 139.658 // 139.658 // module 2
//     public static final Rotation2d kFrontRightOffset = Rotation2d.fromDegrees(318.164); // 139.658 // 139.658
    const frc::Translation2d kFrontRightPosition = frc::Translation2d(units::meter_t {kTrackwidthMeters / 2.0},
            units::meter_t {-kWheelbaseMeters / 2.0});

    const int kBackLeftDriveMotorID = 30;
    const int kBackLeftTurnMotorID = 31;
    const int kBackLeftEncoderID = 32;
    const frc::Rotation2d kBackLeftOffset = frc::Rotation2d(units::degree_t {275.273});  // 95.977 // 96.680 // module 3
//     public static final Rotation2d kBackLeftOffset = Rotation2d.fromDegrees(-84.551);  // 95.977 // 96.680
    const frc::Translation2d kBackLeftPosition = frc::Translation2d(units::meter_t {-kTrackwidthMeters / 2.0},
            units::meter_t {kWheelbaseMeters / 2.0});

    const int kBackRightDriveMotorID = 40;
    const int kBackRightTurnMotorID = 41;
    const int kBackRightEncoderID = 42;
    const frc::Rotation2d kBackRightOffset = frc::Rotation2d(units::degree_t {87.539}); // 265.517 // -93.867 // module 4
//     public static final Rotation2d kBackRightOffset = Rotation2d.fromDegrees(88.770); // 265.517 // -93.867
    const frc::Translation2d kBackRightPosition = frc::Translation2d(units::meter_t {-kTrackwidthMeters / 2.0},
            units::meter_t {-kWheelbaseMeters / 2.0});

    const frc::SwerveDriveKinematics kSwerveKinematics = frc::SwerveDriveKinematics(kFrontLeftPosition,
...
namespace ModuleConstants {
    ...
    const double kDriveGearRatio = 1.0 / GeneralConstants::kSDSModule.driveReduction;
    ...
    const ctre::phoenix::motorcontrol::NeutralMode kDriveMotorNeutral = ctre::phoenix::motorcontrol::NeutralMode::Coast;
    const double kDriveVoltageComp = 12;

    const bool kTurnMotorInverted = true;
    const ctre::phoenix::motorcontrol::NeutralMode kTurnMotorNeutral = ctre::phoenix::motorcontrol::NeutralMode::Brake; // set back to brake to be amazing
    ...
    const frc::Rotation2d kAllowableAngleTolerance = frc::Rotation2d(units::degree_t{5.0});
...
namespace Conversions {
     ...
     */
    inline double falconToMeters(double falconPosition, double circumference, double gearRatio) {
    ...
    inline double falconToMPS(double velocitycounts, double circumference, double gearRatio) {
        double wheelRPM = falconToRPM(velocitycounts, gearRatio);
        double wheelMPS = (wheelRPM * circumference) / kSecondsPerMinute;
        return wheelMPS;
    }

    inline double MPSToFalcon(double velocity, double circumference, double gearRatio) {
