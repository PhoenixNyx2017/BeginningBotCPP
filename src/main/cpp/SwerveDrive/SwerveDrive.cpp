...
SwerveDrive::SwerveDrive() :
    ...
    kinematics{{frc::Translation2d{units::meter_t {GeneralConstants::kTrackwidthMeters/2.0}, 
                                   units::meter_t {GeneralConstants::kWheelbaseMeters/2.0}},
                frc::Translation2d{units::meter_t {GeneralConstants::kTrackwidthMeters/2.0}, 
                                   units::meter_t {-GeneralConstants::kWheelbaseMeters/2.0}},
                frc::Translation2d{units::meter_t {-GeneralConstants::kTrackwidthMeters/2.0}, 
                                   units::meter_t {GeneralConstants::kWheelbaseMeters/2.0}},
                frc::Translation2d{units::meter_t {-GeneralConstants::kTrackwidthMeters/2.0}, 
                                   units::meter_t {-GeneralConstants::kWheelbaseMeters/2.0}}}},
    odometry{kinematics,frc::Rotation2d(units::degree_t {-navx.GetAngle()}),
    ...
    networkTableInst(nt::NetworkTableInstance::GetDefault())
{
    ...
    ntPoseSubscribe = poseTable->GetDoubleArrayTopic(ntName).Subscribe({}, {.periodic = 0.01, .sendAll = true});
...
void SwerveDrive::drive(frc::ChassisSpeeds desiredSpeeds, bool isOpenLoop) {
    ...
    kinematics.DesaturateWheelSpeeds(&states, units::meters_per_second_t {ModuleConstants::kMaxSpeed});
...
void SwerveDrive::setReference(frc::Pose2d desiredPose) {
    ...
    if((!pidX.AtSetpoint() & !pidY.AtSetpoint()) | !hasRun){
        speeds = frc::ChassisSpeeds(
            units::meters_per_second_t {pidX.Calculate(double {getPose().X()}, double {desiredPose.X()})},
            units::meters_per_second_t {pidY.Calculate(double {getPose().Y()}, double {desiredPose.Y()})},
...
std::optional<frc::Pose3d> SwerveDrive::getCameraResults() {
    ...
    if (time != 0.0) {
        ...
        return frc::Pose3d(frc::Translation3d(units::meter_t{compressedResults.at(0)},
