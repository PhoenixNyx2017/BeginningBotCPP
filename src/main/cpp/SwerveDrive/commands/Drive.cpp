...
Drive::Drive(SwerveDrive* i_swerve, std::function<double()> i_x, 
                                    std::function<double()> i_y, 
                                    std::function<double()> i_rot) :
  swerve(i_swerve),
  x(i_x),
  y(i_y),
  rot(i_rot)
...
void Drive::Execute() {
  ...
  if (GeneralConstants::kIsFieldRelative) {
      speeds = frc::ChassisSpeeds::FromFieldRelativeSpeeds(units::meters_per_second_t {x()}, units::meters_per_second_t {y()}, 
                                                          units::radians_per_second_t {rot()}, swerve->getHeading());
  } else {
      speeds = frc::ChassisSpeeds(units::meters_per_second_t {x()}, units::meters_per_second_t {y()}, 
