...
frc::SwerveModuleState SwerveModule::getCurrentState() {
  ...
  return {units::meters_per_second_t{velocity}, angle};
}

frc::SwerveModulePosition SwerveModule::getPosition() {
  double distance = getDistanceMeters();
  frc::Rotation2d angle = getAngleRotation2d();
  return {units::meter_t{distance}, angle};
}
...
void SwerveModule::setDesiredState(frc::SwerveModuleState desiredState,
  ...
  desiredState =
      frc::SwerveModuleState::Optimize(desiredState, currentAngleRotation2d);
...
void SwerveModule::setAngle(frc::SwerveModuleState desiredState,
  ...
  if (std::abs(double{desiredState.speed}) >=
  ...
  if (std::abs(
          double{((currentAngleRotation2d - desiredState.angle).Degrees())}) >
...
void SwerveModule::setVelocity(frc::SwerveModuleState desiredState,
                               bool isOpenLoop) {
  if (isOpenLoop) {
    double percentOutput =
        double{desiredState.speed} / ModuleConstants::kMaxSpeed;
    driveMotor.Set(ctre::phoenix::motorcontrol::ControlMode::PercentOutput,
                   percentOutput);
  } else {
    double velocity = Conversions::MPSToFalcon(
        double{desiredState.speed}, ModuleConstants::kWheelCircumference,
        ModuleConstants::kDriveGearRatio);
    double ffCalculate =
        ModuleConstants::kDriveS *
            static_cast<double>(wpi::sgn(double{desiredState.speed})) +
...
void SwerveModule::initDriveMotor(int driveMotorID) {
  ...
  kDriveMotorConfig.initializationStrategy =
      ctre::phoenix::sensors::SensorInitializationStrategy::BootToZero;
  ...
  kDriveMotorConfig.velocityMeasurementPeriod =
      ctre::phoenix::sensors::SensorVelocityMeasPeriod::Period_10Ms;
...
void SwerveModule::initEncoder(int encoderID) {
  ...
  kEncoderConfig.initializationStrategy = ctre::phoenix::sensors::
      SensorInitializationStrategy::BootToAbsolutePosition;
  kEncoderConfig.sensorTimeBase =
...
void SwerveModule::updateSmartDash() {
  ...
  frc::SmartDashboard::PutNumber(std::to_string(id) + "PureRaw Angle",
                                 double{getAbsoluteAngle().Degrees()});
...
SwerveModule::checkForWrapAround(frc::SwerveModuleState desiredState,
  ...
  double speedMulti;
  double remainder =
      std::remainder(desiredDegrees - double{currentState.Degrees()}, 180);
...
frc::Rotation2d SwerveModule::getAngleRotation2d() {
  return frc::Rotation2d(units::degree_t{
      Conversions::falconToDegrees(steerMotor.GetSelectedSensorPosition(),
